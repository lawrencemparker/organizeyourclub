...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-[#0B0F1A] border-b border-white/10 z-50 flex items-center justify-between px-4">
        <div className="flex items-center gap-2">
           <div className="w-8 h-8 rounded-lg bg-[var(--primary)] flex items-center justify-center text-white font-bold">
             {orgData?.name && orgData.name !== "No Organization" && orgData.name !== "Connection Error" ? orgData.name.substring(0, 2).toUpperCase() : "OC"}
           </div>
           <span className="font-bold text-white text-sm truncate max-w-[180px]">{orgData?.name || "Loading..."}</span>
        </div>
      ...
      {/* THE SIDEBAR */}
      <div className={cn(
        "fixed md:static inset-y-0 left-0 z-50 transform transition-transform duration-300 ease-in-out bg-[#0B0F1A] border-r border-border/40 w-64 flex flex-col h-full shrink-0",
        isMobileMenuOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
      )}>
        
        {/* ORGANIZATION SWITCHER */}
        <div className="p-4 border-b border-white/5 mt-16 md:mt-0">
          <DropdownMenu>
            ...
            visibleMenuItems.map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  onClick={() => setIsMobileMenuOpen(false)} // Close menu on tap
                  className={cn(
                    "flex items-center gap-3 px-3 py-3 md:py-2 rounded-lg text-sm font-medium transition-all group",
                    isActive 
                      ? "bg-[var(--primary)]/10 text-[var(--primary)]" 
                      : "text-muted-foreground hover:bg-white/5 hover:text-foreground"
                  )}
                >
                  <item.icon className={cn("w-5 h-5 md:w-4 md:h-4 transition-colors", isActive ? "text-[var(--primary)]" : "group-hover:text-foreground")} />
                  {item.label}
                </Link>
              );
            })
