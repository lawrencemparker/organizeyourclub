...
import { 
  ...
  Calendar, Info, ChevronLeft, Plus, MoreVertical, Edit2, Trash2 
} from "lucide-react";
...
export function CompliancePage() {
  ...
  return (
    <div className="p-3 sm:p-5 space-y-3"> 
      {/* Clean Navigation - Redundant links removed */}
           <div className="flex items-center justify-end">
        <Button 
          variant="outline" 
          size="sm" 
          onClick={() => { setEditingTask(null); setIsFormOpen(true); }} 
          className="h-8 border-[var(--primary)]/20 text-[var(--primary)] hover:bg-[var(--primary)]/5 text-xs font-bold px-4 transition-colors"
        >
      ...
      {/* BRANDING FIX: Overall Progress now uses var(--primary) */}
      <div className="glass-card p-3 border-l-4 border-[var(--primary)] flex items-center justify-between gap-4 h-16 transition-colors duration-300">
        <div className="flex-1">
          ...
          <p className="text-[10px] font-medium text-muted-foreground">{stats.completed} of {stats.total} items completed</p>
        </div>
        <div className="flex items-center gap-4">
          <span className="text-2xl font-black tracking-tighter text-[var(--primary)]">{stats.percent}%</span>
          <div className="w-24 h-1.5 bg-[var(--primary)]/10 rounded-full overflow-hidden">
            <div 
            ...
            <div key={task.id} className="glass-card overflow-hidden transition-all border-border/10 group">
              <div className="p-2.5 flex items-center justify-between h-14"> 
                <div className="flex items-center gap-3 min-w-0 flex-1 cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : task.id)}>
                  ...
                  <div className="min-w-0">
                    <h4 className="text-sm font-semibold truncate group-hover:text-[var(--primary)] transition-colors">{task.title}</h4>
                    <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
                      <Calendar className="w-3 h-3 opacity-60" /> 
                      <span className="font-medium">Due: {task.due_date}</span>
                    </div>
                  </div>
                </div>

                <div className="flex items-center gap-2">
                  <Badge variant="secondary" className={cn("text-[9px] h-5 px-2 capitalize font-bold border-transparent", 
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="icon" className="h-7 w-7 opacity-30 group-hover:opacity-100 transition-opacity">
              ...
              {isExpanded && (
                <div className="px-9 pb-3 animate-in slide-in-from-top-2 duration-200">
                  <div className="p-3 rounded-lg bg-white/5 border border-white/5 text-[11px] leading-relaxed text-muted-foreground italic">
                    <div className="flex items-center gap-1.5 mb-1 text-foreground font-bold not-italic">
                      <Info className="w-3 h-3 text-[var(--primary)]" /> Requirement Details
                    </div>
